...
toggle() {
    if [[ -n "$1" ]]; then
        ...
        # Initialize gpu_flags and priority GPU if they don't exist
    ...
    # Set the next priority GPU and remove the '#' character
    sed -i 's/^\(nvidia_flag=1\|amd_flag=1\|intel_flag=1\)/#\1/' "${gpuQ}" # Comment out all the GPU flags in the file
    sed -i "s/^#${next_prioGPU}/${next_prioGPU}/" "${gpuQ}" # Uncomment the next priority GPU in the file
    sed -i "s/prioGPU=${prioGPU}/prioGPU=${next_prioGPU}/" "${gpuQ}" # Update the priority GPU in the file
}
...
get_icons() { # Key-value pairs of temperature and utilization levels
...
generate_json() { # Get emoji and icon based on temperature and utilization

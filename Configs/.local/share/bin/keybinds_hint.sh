...
#? Autogenerated comment conversion
$comments
#? Defaults
" empty " :  "Empty",
"r+1" : "Relative Right",
...
include "hyprdots-keybinds";
  ...
  def arg_mapping: { #! Do not change this used for demo only... as this will change .args
    "arg2": "mapped_arg2",
  };
    ...
    "movewindow" : "Move Window",
    "exec" : "" , #? Remove exec as executable will give the description from a separate function
...
DISPLAY() {
  awk -v kb_hint_delim="${kb_hint_delim:->}" -F '!=!' '{
    if ($0 ~ /=/ && $6 != "")
      printf "%-25s %-2s %-30s\n", $5, kb_hint_delim, $6;
    else if ($0 ~ /=/)
      printf "%-25s\n", $5;

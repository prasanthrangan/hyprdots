...
# Function to check for AMD GPU using 'lspci'
check_amd_gpu() {
  local amd_gpu
  amd_gpu=$(lspci -nn | grep -E "VGA|3D controller" | grep -i "AMD")
  echo "$amd_gpu"
}
...
get_intel_gpu_temperature() {
  local temperature
  temperature=$(sensors | grep "coretemp-isa-0000" | grep "Package id 0" | awk '{print $4}' | sed 's/+//')
  echo "$temperature"
}

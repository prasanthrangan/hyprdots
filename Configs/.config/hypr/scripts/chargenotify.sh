...
mnc=5 mxc=80 mnl=20 mxl=80 mnu=80  mxu=100 mnt=60 mxt=1000 mnf=80 mxf=100 mnn=1 mxn=1140 mni=1 mxi=10 verbose=false undock=false #Defaults Ranges
while (( "$#" )); do  # Parse command-line arguments and defaults  
  case "$1" in
    ...
    *|"--help"|"-h")
      ...
      echo "  --notify, -n      Set notify interval for Battery Full Status  (default: $mxn mins/ 1 day)"
...
is_laptop() { if grep -q "Battery" /sys/class/power_supply/BAT*/type; then return 0  # It's a laptop
    else echo "Cannot Detect a Battery. If this seems an error please report an issue to https://github.com/prasanthrangan/hyprdots."
        exit 0  # It's not a laptop
    fi
}
fn_verbose () { if $verbose; then 
cat << VERBOSE
=============================================
...
fn_notify () { notify-send -a "Power" $1 -u $2 "$3" "$4" -p ; } # Call the notify-send command with the provided arguments \$1 is the flags \$2 is the urgency \$3 is the title \$4 is the message 
fn_percentage () { 
...
fn_action () { count=$(( timer > $mnt ? timer :  $mnt )) ; nohup $execute ;}
fn_status () {
if [[ $battery_percentage -ge $battery_full_threshold && $battery_status == *"Charging"* ]]; then echo "Full and $battery_status" ; battery_status="Full" ;fi
case "$battery_status" in         # Handle the power supply status
                ...
                #echo p$prev_status s$battery_status
                ...
                "Not"*|"Charging") if $verbose; then echo "Case:$battery_status Level: $battery_percentage" ;fi
                    if [[ "$prev_status" != *"Charging"* ]] || [[ "$prev_status" == "Not"* ]] ; then
...
fn_verbose
fn_percentage
if $undock; then fn_status; fi ;fi ;done
}
resume_processes() { for pid in $pids ; do  if [ $pid -ne $current_pid ] ; then kill -CONT $pid ; notify-send -a "Power" -t 2000 -r 9889 -u "CRITICAL" "Debugging ENDED, Resuming Regular Process" ; fi ; done }
main() { if is_laptop; then pkill -f "/bin/bash $HOME/.config/hypr/scripts/batterynotify.sh"  # takeover
...
current_pid=$$
pids=$(pgrep -f "/bin/bash $HOME/.config/hypr/scripts/chargenotify.sh" )
for pid in $pids ; do if [ $pid -ne $current_pid ] ;then kill -STOP $pid ;notify-send -a "Power" -t 2000 -r 9889 -u "CRITICAL" "Debugging STARTED, Pausing Regular Process" ;fi ; done  ; trap resume_processes SIGINT ; fi
last_notified_percentage=$(cat /sys/class/power_supply/BAT*/capacity) prev_status=$(cat /sys/class/power_supply/BAT*/status)

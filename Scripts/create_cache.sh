...
imagick_t2 () {
    ...
    if [ ! -f "${cacheDir}/${wpBaseName}" ]; then
        convert "${wpFullName}"[0] -thumbnail 500x500^ -gravity center -extent 500x500 "${cacheDir}/${wpBaseName}"
    fi

    if [ ! -f "${cacheDir}/${wpBaseName}.rofi" ]; then
        convert -strip -resize 2000 -gravity center -extent 2000 -quality 90 "${wpFullName}"[0] "${cacheDir}/${wpBaseName}.rofi"
    fi

    if [ ! -f "${cacheDir}/${wpBaseName}.blur" ]; then
        convert -strip -scale 10% -blur 0x3 -resize 100% "${wpFullName}"[0] "${cacheDir}/${wpBaseName}.blur"
    fi
